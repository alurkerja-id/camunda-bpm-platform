...
public class ActivityInstanceJobHandler implements MigratingDependentInstanceParseHandler<MigratingActivityInstance, List<JobEntity>> {
  ...
  protected boolean isNonInterruptingTimerTriggeredAlready(MigratingInstanceParseContext parseContext,
    ...
    if (targetTimerDeclaration.isInterruptingTimer() || !TimerExecuteNestedActivityJobHandler.TYPE.equals(targetTimerDeclaration.getJobHandlerType()) || sourceTimerDeclarationsInEventScope.values().isEmpty()) {
      return false;
    }
  ...
  protected boolean isNonInterruptingTimeoutListenerTriggeredAlready(MigratingInstanceParseContext parseContext,
    ...
    if (targetTimerDeclaration.isInterruptingTimer() || !TimerTaskListenerJobHandler.TYPE.equals(targetTimerDeclaration.getJobHandlerType()) || sourceTimeoutListenerDeclarationsInEventScope.values().isEmpty()) {
      return false;
    }
